...
const API = localStorage.getItem('aether_agent_url') || 'http://localhost:3000';
...
// ─── HOSTED MODE: static library ingestion ────────────────────────────────────
// Called by vault/index.html when agent is offline — loads data/all_compositions.json
// directly from GitHub Pages, enabling browse/sheet-music without local agent.
window._ingestLibrary = function(data, agentLive = false) {
    ...
    const composers = new Set(library.map(i => i.composer || '?')).size;
    const midis = library.filter(i => i.midi === true).length;
    const missing = library.filter(i => i.midi !== true).length;
    ['stat-composers', 'sb-stat-composers'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = composers;
    });
    ['stat-songs', 'sb-stat-songs'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = library.length;
    });
    ['stat-midis', 'sb-stat-midis'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = midis;
    });
    const missingEl = document.getElementById('sb-stat-missing');
    if (missingEl) missingEl.textContent = missing;
    const badge = document.getElementById('mode-badge');
    if (badge) badge.textContent = agentLive ? 'LOCAL AGENT LIVE' : 'HOSTED MODE — HARVEST DISABLED';
};
