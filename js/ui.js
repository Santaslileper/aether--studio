...
export function setupUI(handlers) {
    ...
    searchInput.addEventListener('input', e => {
        ...
        searchTimer = setTimeout(async () => {
            ...
            try {
                ...
                const apiBase = isLocal ? (location.origin.includes('3000') ? '' : 'http://localhost:3000') : '';
                const res = await fetch(`${apiBase}/api/search?q=${encodeURIComponent(term)}`);
        ...
        if (item.dataset.path || item.dataset.url) {
            ...
            try {
                ...
                if (url) {
                    ...
                    const isLocal = ['http:', 'file:'].includes(location.protocol);
                    const apiBase = isLocal ? 'http://127.0.0.1:3000' : '';
    ...
    document.getElementById('settings-open').addEventListener('click', () => {
        ...
        h += '</div>';
        list.innerHTML = h;
        const kbToggle = document.getElementById('toggle-kb-link');
        if (kbToggle) kbToggle.checked = state.keyboardLinkEnabled;
        const kbPlay = document.getElementById('kb-link-play');
        if (kbPlay) kbPlay.checked = state.kbLinkOnPlay;
        const kbGuide = document.getElementById('kb-link-guide');
        if (kbGuide) kbGuide.checked = state.kbLinkOnGuide;
    ...
    document.getElementById('toggle-kb-link')?.addEventListener('change', e => {
        state.keyboardLinkEnabled = e.target.checked;
        lsSet('kbLink', state.keyboardLinkEnabled);
        if (state.keyboardLinkEnabled) initKeyboardLink();
        else if (state.rgbSocket) { state.rgbSocket.disconnect(); state.rgbSocket = null; }
    });
    ...
    document.getElementById('kb-link-play')?.addEventListener('change', e => {
        state.kbLinkOnPlay = e.target.checked;
        lsSet('kbLinkPlay', state.kbLinkOnPlay);
    });

    document.getElementById('kb-link-guide')?.addEventListener('change', e => {
        state.kbLinkOnGuide = e.target.checked;
        lsSet('kbLinkGuide', state.kbLinkOnGuide);
    });
    ...
    function initKeyboardLink() {
        if (!state.keyboardLinkEnabled || state.rgbSocket) return;
        console.log('[KB Link] Connecting to', state.rgbServerUrl);
        state.rgbSocket = io(state.rgbServerUrl);
        state.rgbSocket.on('connect', () => console.log('[KB Link] Connected to RGB Studio'));
        state.rgbSocket.on('connect_error', () => console.warn('[KB Link] Connection failed'));
    }

    if (state.keyboardLinkEnabled) setTimeout(initKeyboardLink, 1000);
